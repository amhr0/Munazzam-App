...
// This is similar to meet-content.js but adapted for Zoom's UI

console.log('[Munazzam Zoom] Content script loaded');

// Import the same functionality from meet-content but with Zoom-specific selectors
// Most of the logic is the same, only UI selectors differ
...
// Initialize
async function init() {
  console.log('[Munazzam Zoom] Initializing...');
...
function waitForMeeting() {
  const checkInterval = setInterval(() => {
    // Check if we're in a meeting (Zoom video container exists)
    const videoContainer = document.querySelector('[class*="video-container"]') || 
                          document.querySelector('[class*="gallery-video-container"]');
    if (videoContainer) {
      console.log('[Munazzam Zoom] Meeting detected');
...
async function onMeetingReady() {
  console.log('[Munazzam Zoom] Meeting is ready');
...
function observeMeetingStart() {
  const titleObserver = new MutationObserver(() => {
    ...
    if (title && !isRecording) {
      console.log('[Munazzam Zoom] Meeting started:', title);
      startRecording();
    }
  });
...
// Get meeting title (Zoom-specific)
function getMeetingTitle() {
  // Try Zoom-specific selectors
  const selectors = [
    '[class*="meeting-topic"]',
    '[class*="meeting-title"]',
    '[aria-label*="meeting"]'
  ...
  return document.title.replace(' - Zoom', '').trim() || 'اجتماع Zoom';
}

// Start recording (same as Meet)
async function startRecording() {
  if (isRecording) return;
  
  try {
    console.log('[Munazzam Zoom] Starting recording...');
    ...
    mediaRecorder.onstop = () => {
      console.log('[Munazzam Zoom] Recording stopped');
    ...
    console.log('[Munazzam Zoom] Recording started successfully');
  } catch (error) {
    console.error('[Munazzam Zoom] Error starting recording:', error);
...
function stopRecording() {
  ...
  console.log('[Munazzam Zoom] Stopping recording...');
  mediaRecorder.stop();
...
// Connect to Socket.io (same as Meet)
function connectToSocket(serverUrl) {
  console.log('[Munazzam Zoom] Connecting to Socket.io:', serverUrl);
  ...
  script.onload = () => {
    ...
    socket.on('connect', () => {
      console.log('[Munazzam Zoom] Socket.io connected');
      updateSidebarStatus('connected');
    });
    
    socket.on('disconnect', () => {
      console.log('[Munazzam Zoom] Socket.io disconnected');
    });
...
function startSocketSession() {
  ...
  socket.once('session-started', (data) => {
    ...
    console.log('[Munazzam Zoom] Socket session started:', sessionId);
  });
}
...
// Inject sidebar (same HTML as Meet)
...
// Update sidebar status (same as Meet)
...
// Add transcription/suggestions/flags (same as Meet)
